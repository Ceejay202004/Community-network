(function() {
    ...
    function setActiveTab(tab) {
        [tabPosts, tabMembers, tabMessages, tabCreate, tabPending].forEach(t => {
    ...
    function updateStats() {
        ...
        onlineUsers.forEach(u => {
            onlineHtml += `
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px; cursor: pointer;" onclick="openChat('${u.email}')">
                    <div class="status-dot" style="background: #22c55e;"></div>
                    <span style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${u.fullName}</span>
                </div>
            `;
        });
